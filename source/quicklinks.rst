============================================
Quick Links: User Guide Videos
============================================

Welcome to the iNoCulator video library. Browse the categories below to find step-by-step video walkthroughs for account setups, project management, and hardware configuration workflows.

--------------------------------------------------------------------------------

Project Level Essentials
========================

.. grid:: 2
   :gutter: 3

   .. grid-item-card:: Introduction & Navigation
      :columns: 6

      Learn the basics of the iNoCulator platform and how to navigate the AI-driven dashboard.
      
      +++
      .. button-link:: https://youtu.be/j0w4Fm2YtUY
         :color: primary
         :expand:

         🍿 Watch Introduction Video

   .. grid-item-card:: Account Access & Activation
      :columns: 6

      A complete walkthrough on how to request system access, activate your profile, and log in securely.
      
      +++
      .. button-link:: https://youtu.be/60J9YAYPHNo
         :color: primary
         :expand:

         🍿 Watch Access Guide

   .. grid-item-card:: Creating & Managing Projects
      :columns: 6

      Master project creation, general lifecycle management, and setting up your initial workspaces.
      
      +++
      .. button-link:: https://youtu.be/wj6lU_nhcK4
         :color: primary
         :expand:

         🍿 Watch Management Guide

   .. grid-item-card:: Collaboration: Share, Copy & Edit
      :columns: 6

      Learn how to share architectures with team members, duplicate layouts, and modify existing project files.
      
      +++
      .. button-link:: https://youtu.be/3YhIcV8UTrg
         :color: primary
         :expand:

         🍿 Watch Collaboration Guide

--------------------------------------------------------------------------------
Non-Coherent NoC (NC-NoC)
=========================

.. grid:: 2
   :gutter: 3

   .. grid-item-card:: NC-NoC: Step-by-Step Creation
      :columns: 6

      {bdg-primary}`NC-NoC`

      A complete walkthrough of creating your very first Non-Coherent NoC design workspace from scratch.
      
      +++
      .. button-link:: https://youtu.be/B_i1tYkxjJo
         :color: secondary
         :expand:

         🍿 Watch Setup Guide

   .. grid-item-card:: Device Configuration
      :columns: 6

      {bdg-primary}`NC-NoC`

      Detailed guide on setting up, dimensioning, and tailoring Non-Coherent devices within your fabric topology.
      
      +++
      .. button-link:: https://youtu.be/nX_n8KM3I7U
         :color: secondary
         :expand:

         🍿 Watch Configuration Guide

   .. grid-item-card:: Generating & Downloading Designs
      :columns: 6

      {bdg-primary}`NC-NoC`

      How to compile your architecture specifications, run validation, and download your finalized generation outputs.
      
      +++
      .. button-link:: https://youtu.be/-3HMNqfc0qA
         :color: secondary
         :expand:

         🍿 Watch Generation Guide

   .. grid-item-card:: Package Export Workflow
      :columns: 6

      {bdg-primary}`NC-NoC`

      Step-by-step instructions on cleanly exporting and downloading structured packages out of Inoculator.
      
      +++
      .. button-link:: https://youtu.be/QsZHJD1lKTQ
         :color: secondary
         :expand:

         🍿 Watch Export Guide

--------------------------------------------------------------------------------

Coherent NoC (C-NoC)
====================

.. grid:: 2
   :gutter: 3

   .. grid-item-card:: C-NoC: Step-by-Step Creation
      :columns: 6

      {bdg-dark}`C-NoC`

      An end-to-end framework guide showing you how to initiate a Coherent NoC architecture project.
      
      +++
      .. button-link:: https://youtu.be/M5oFPE3sQ_U
         :color: dark
         :expand:

         🍿 Watch Setup Guide

   .. grid-item-card:: Coherent Fabric Configuration
      :columns: 6

      {bdg-dark}`C-NoC`

      Learn to define protocols, coherency directories, and map advanced fabric parameters.
      
      +++
      .. button-link:: https://youtu.be/ztJ1ydKA6eI
         :color: dark
         :expand:

         🍿 Watch Fabric Guide

   .. grid-item-card:: Export & Download Design
      :columns: 6

      {bdg-dark}`C-NoC`

      Consolidate your coherent design parameters, trigger the compiler, and pull your deliverables safely down.
      
      +++
      .. button-link:: https://youtu.be/ztJ1ydKA6eI
         :color: dark
         :expand:

         🍿 Watch Export Guide
